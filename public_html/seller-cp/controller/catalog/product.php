...
class ControllerCatalogProduct extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->request->post['tax_class_id'] = 0;
            $this->request->post['manufacturer_id'] = 0;
            $this->request->post['product_store'] = [0];
	...
	public function edit() {
		...
		$this->load->model('catalog/product');
        $getProduct = $this->model_catalog_product->getProduct($this->request->get['product_id']);
        if(empty($getProduct)){
            $this->response->redirect($this->url->link('catalog/product/add', 'user_token=' . $this->session->data['user_token'], true));
        }
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			if($result['status']==1){
                $status = '<span class="label label-success">'.$this->language->get('text_enabled').'</span>';

            }elseif ($result['status'] ==2){
                $status = '<span class="label label-warning">'.$this->language->get('text_waitting_product').'</span>';
            }else{
                $status = '<span class="label label-danger">'.$this->language->get('text_disabled').'</span>';
            }
	...
	protected function getForm() {
		...
		if (isset($this->request->post['product_store'])) {
			...
			$data['product_store'] = [$this->seller->getStoreId()];
		}
		...
		} elseif (!empty($product_info)&& !empty($product_info)) {
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['product_description'] as $language_id => $value) {
            ...
            if ((utf8_strlen(strip_tags($value['description'])) < 10)) {
                $this->error['description'][$language_id] = 'يجب اضافة وصف للمنتج';
            }
		...
		if ((utf8_strlen($this->request->post['price']) < 1) || (utf8_strlen($this->request->post['price']) > 64)) {
			$this->error['price'] = 'لا يمكن ترك السعر فارغ';
		}
		if (!isset($this->request->post['product_category']) || empty($this->request->post['product_category'])) {
			$this->error['category'] = 'يجب تحديد المنتجات';
		}
		if ((utf8_strlen($this->request->post['image']) < 1)) {
			$this->error['image'] = 'يجب اختيار صورة  للمنتج';
		}
		if (!isset($this->request->post['product_image']) || empty($this->request->post['product_image'])) {
			//$this->error['images'] = 'يجب اختيار صور إضافية للمنتج';
		}
		if (!is_numeric($this->request->post['location'])) {
			$this->error['location'] = 'الوقت المستغرق يجب ان يكون ارقام فقط';
		}
