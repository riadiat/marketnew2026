...
class ControllerAccountOrder extends Controller {
	...
	public function info() {
		...
		if ($order_info) {
			...
			foreach ($ids as $id){
			    $active = false;
                $getOrderStatus = $this->model_localisation_order_status->getOrderStatus($id);
                if($getOrderStatus){
                   if($order_info['order_status_id'] == 2){
                       if($getOrderStatus['order_status_id'] == 2){
                           $active = true;
                       }
                   }
                    if($order_info['order_status_id'] == 6){
                        if($getOrderStatus['order_status_id'] == 2 || $getOrderStatus['order_status_id'] == 6){
                            $active = true;
                        }
                    }
                    if($order_info['order_status_id'] == 3){
                        if($getOrderStatus['order_status_id'] == 2 || $getOrderStatus['order_status_id'] == 6 || $getOrderStatus['order_status_id'] == 3){
                            $active = true;
                        }
                    }
                    $data['orders_status'] []=[
                        'order_status_id'=>$getOrderStatus['order_status_id'],
