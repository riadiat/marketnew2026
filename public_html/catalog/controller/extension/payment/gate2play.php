...
class ControllerExtensionPaymentGate2play extends Controller {
    ...
    public function index() {
        $this->language->load('extension/payment/gate2play');
	$this->load->model('checkout/order');
        $data['button_confirm'] = $this->language->get('button_confirm');
        //--------------------------------------
        $testMode = $this->config->get('payment_gate2play_testmode');
        if ($testMode == 0) {
        ...
        $channel = trim($this->config->get('payment_gate2play_channel'));
        $mode = $this->config->get('payment_gate2play_trans_mode');
        $login = trim($this->config->get('payment_gate2play_loginid'));
        $pwd = trim($this->config->get('payment_gate2play_password'));
        $type = $this->config->get('payment_gate2play_trans_type');
        $amount = number_format(round($orderAmount, 2), 2, '.', '');
        $currency = $this->config->get('payment_gate2play_base_currency');
        ...
        $payment_brands = implode(' ', $this->config->get('payment_gate2play_brands'));
        //--------------------------------------
        $data['token'] = $token;
        $data['payment_brands'] = $payment_brands;
        $data['scriptURL'] = $scriptURL . $token;

        $data['formStyle'] = $this->config->get('payment_gate2play_payment_style');
        ...
        return $this->load->view('extension/payment/gate2play', $data);
    ...
    public function callback() {
        if (isset($_GET['id'])) {
            ...
            $url .= "?authentication.userId=" . trim($this->config->get('payment_gate2play_loginid'));
            $url .= "&authentication.password=" . trim($this->config->get('payment_gate2play_password'));
            $url .= "&authentication.entityId=" . trim($this->config->get('payment_gate2play_channel'));
        ...
        if(isset($resultJson->resultDetails->ErrorMessage)){
            ...
            if ($order_info) {
                if ($success == 1) {
                    ...
                    $this->model_checkout_order->addOrderHistory($orderid, $this->config->get('payment_gate2play_order_status_id'), "Trans Unique ID:$transUniqueID\n", TRUE);
                    $this->success();
                } else {
                    // Order is not approved.
                    $this->model_checkout_order->addOrderHistory($orderid, $this->config->get('payment_gate2play_order_status_failed_id'), '', TRUE);
                    $this->log->write("Hyperpay: Unauthorized Transaction. Transaction Failed. $failed_msg . Order Id: $orderid");
                    $this->session->data['payment_gate2play_error'] = $failed_msg;
                    $this->response->redirect($this->url->link('extension/payment/gate2play/fail', '', true));
                }
                exit;
            } else {
                if ($this->config->get('payment_gate2play_mailerrors') == 1) {
                ...
                //$this->model_checkout_order->confirm($orderid, $this->config->get('payment_gate2play_order_status_failed_id'), '', TRUE);
                $this->model_checkout_order->addOrderHistory($orderid, $this->config->get('payment_gate2play_order_status_failed_id'), '', TRUE);
                $this->log->write("Hyperpay: Unauthorized Transaction. Transaction Failed. $failed_msg. Order Id: $orderid");
                $this->response->redirect($this->url->link('extension/payment/gate2play/fail', '', true));
                print 'fff';
    ...
    public function fail() {
        ...
        if (isset($this->session->data['payment_gate2play_error'])) {
            $data['general_error'] = $this->session->data['payment_gate2play_error'];
        } else {
        ...
        $data['header'] = $this->load->controller('common/header');
            ...
            $this->response->setOutput($this->load->view('extension/payment/gate2play_fail', $data));
