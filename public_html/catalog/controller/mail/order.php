...
class ControllerMailOrder extends Controller {
	...
	private function notifySeller($order_id){
        ...
        foreach ($order_products as $order_product){
            ...
            if($getSeller){
                $mail->setText($this->load->view('mail/order_alert', $data));
                $mail->setTo($getSeller['email']);
                $mail->send();


                $this->load->model('checkout/sms');
                $url = "https://saudiewstore.tech-station.co/seller-cp/index.php?route=sale/order/info&order_id=".$order_info['order_id'];
                $msg = 'طلب رقم #'.$order_info['order_id']." \n ".$url;
                $this->model_checkout_sms->sendSms($msg, $getSeller['telephone']);
            }
