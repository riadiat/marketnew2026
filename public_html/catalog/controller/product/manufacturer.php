    ...
    public function index() {
        ...
        foreach ($results as $manufacturer){
            if($manufacturer['image']&& is_file(DIR_IMAGE . $manufacturer['image'])){
                $thumb = $this->model_tool_image->resize($manufacturer['image'], 300, 300);
            }else{
                $thumb = $this->model_tool_image->resize('catalog/theme/placeholder.png', 300, 300);
            }
            $data['manufacturers'][]=[
        ...
        if ($manufacturer_info) {
            ...
            foreach ($results as $result) {
                if ($result['image'] && is_file(DIR_IMAGE . $result['image'])) {
                    ...
                    $image = $this->model_tool_image->resize('catalog/theme/placeholder.png', $this->config->get('theme_' . $this->config->get('config_theme') . '_image_product_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_product_height'));
                }
                ...
                if ((float)$result['special']) {
                    ...
                    $price_1 = $this->currency->format($result['price'], $this->session->data['currency'], '', false);
                    $price_special = $this->currency->format($result['special'], $this->session->data['currency'], '', false);
                    $percent = $this->currency->calculatePercentage($price_1,$price_special);
                } else {
                    $special = false;
                }
