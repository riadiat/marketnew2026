...
class ModelAccountCustomer extends Model {
	...
	// public function getCustomerByEmail($email) {
	// 	$query = $this->db->query("SELECT c.* , ca.type as approved FROM " . DB_PREFIX . "customer as c LEFT JOIN  " . DB_PREFIX . "oc_customer_approval as ca ON (ca.customer_id = c.customer_id)  WHERE LOWER(c.email) = '" . $this->db->escape(utf8_strtolower($email)) . "'");
    ...
    public function add_email($email){
        $sql = 'INSERT INTO ' . DB_PREFIX . 'newsletter SET email="'.$this->db->escape($email).'"';
        $this->db->query($sql);
    }
    public function get_email($email){
        $sql = 'SELECT * FROM ' . DB_PREFIX . 'newsletter WHERE email="'.$this->db->escape($email).'"';
