...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
			    $getProduct = $this->db->query("SELECT seller_id FROM " . DB_PREFIX . "product WHERE product_id ='".(int)$product['product_id']."'");
                $seller_id = $getProduct->row['seller_id'];
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "',seller_id = '".(int)$seller_id."'");
	...
	public function addOrderHistorySeller($order_id, $order_status_id, $comment = '', $notify = false, $override = false,$seller_id) {
        $order_info = $this->getOrder($order_id);
        if ($order_info) {
            $this->db->query("INSERT INTO " . DB_PREFIX . "order_history_seller SET order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$order_status_id . "', notify = '" . (int)$notify . "', comment = '" . $this->db->escape($comment) . "',seller_id = '".(int)$seller_id."', date_added = NOW()");
        }
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
            $this->load->model('checkout/sms');
            //$this->model_checkout_sms->send_sms($order_id, $order_status_id,$comment);
