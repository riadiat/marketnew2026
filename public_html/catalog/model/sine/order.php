...
class ModelSineOrder extends Model {
    ...
    public function addAmountOrder($total_admin_sub,$total_seller_sub,$order,$seller_id){
        ...
        if(!$query->row){
            $this->db->query("INSERT INTO `" . DB_PREFIX . "seller_amount` SET seller_id='".$seller_id."' , order_id = '".$order_id."', price='".(float)$total_seller_sub."'");
        }
        if(!$query2->row){
            $this->db->query("INSERT INTO `" . DB_PREFIX . "admin_amount` SET seller_id='".$seller_id."' , order_id = '".$order_id."', price='".(float)$total_admin_sub."'");
        }
    }
