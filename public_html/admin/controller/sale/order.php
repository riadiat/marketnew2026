...
class ControllerSaleOrder extends Controller {
	...
	public function info() {
		...
		if ($order_info) {
            ...
            $ak_smsa = $this->model_sale_smsa->getSmsaByOrder($order_info['order_id']);
            if($ak_smsa){
                $data['shipping_track_no_n'] = $ak_smsa;
            }else{
                $data['shipping_track_no_n'] = false;
            }
			...
			foreach ($products as $product) {
				...
				$seller = $this->model_seller_user->getUser(@$product_info['seller_id']);
				...
				$data['products'][] = array(
					...
					'seller'    	   => [
					    ...
					    'link'=>$this->url->link('seller/user/edit', 'user_token=' . $this->session->data['user_token'] . '&user_id=' . @$seller['user_id'], true),
                        'store'=>HTTPS_CATALOG.@$seller['username'],
                    ],
					'model'    		   => $product['model'],
					'option'   		   => $option_data,
					'quantity'		   => $product['quantity'],
            ...
            $data['order_statuses2'] = [];
            $order_statuses = $this->model_localisation_order_status->getOrderStatuses();
            foreach ($order_statuses as $order_status){
                if($order_status['order_status_id']==7 || $order_status['order_status_id'] ==8){
                    $data['order_statuses2'] [] =[
            ...
            $this->load->model('seller/user');
            $histories = $this->model_sale_order->getOrderHistoriesSeller($order_info['order_id'], 0, 999999);
            $data['sellers'] = [];
            if($histories){
                foreach ($histories as $history){
                    $getUser = $this->model_seller_user->getUser($history['seller_id']);
                    $data['sellers'][$history['seller_id']] = $getUser;
                }
            }
	...
	public function history_seller() {
		...
		$results = $this->model_sale_order->getOrderHistoriesSeller($this->request->get['order_id'], ($page - 1) * 10, 10);

		foreach ($results as $result) {
		    $getUser = $this->model_seller_user->getUser($result['seller_id']);
			$data['histories'][] = array(
				'notify'     => $result['notify'] ? $this->language->get('text_yes') : $this->language->get('text_no'),
				'status'     => $result['status'],
				'seller'     => $getUser,
		...
		$history_total = $this->model_sale_order->getTotalOrderHistoriesSeller($this->request->get['order_id']);

		$pagination = new Pagination();
		$pagination->total = $history_total;
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('sale/order/history_seller', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $this->request->get['order_id'] . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($history_total) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($history_total - 10)) ? $history_total : ((($page - 1) * 10) + 10), $history_total, ceil($history_total / 10));

		$this->response->setOutput($this->load->view('sale/order_history_seller', $data));
	}
