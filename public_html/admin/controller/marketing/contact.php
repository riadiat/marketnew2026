			...
			if (!$json) {
				...
				switch ($this->request->post['to']) {
                    ...
                    case 'seller_all':
                        ...
                        $email_total = $this->model_seller_user->getTotalUsers($user_data);
                        $results = $this->model_seller_user->getUsers($user_data);
                        foreach ($results as $result) {
                            $emails[] = $result['email'];
                        }
                        ...
                        break;
                    case 'seller_enabled':
                        $user_enabled_data = array(
                            'filter_status'=> 1,
                            'start' => ($page - 1) * 10,
                            'limit' => 10
                        );
                        $email_total = $this->model_seller_user->getTotalUsers($user_enabled_data);
                        $results = $this->model_seller_user->getUsers($user_enabled_data);
                        foreach ($results as $result) {
                            $emails[] = $result['email'];
                        }
                    ...
                    case 'seller_disabled':
                        $seller_disabled_data = array(
                            'filter_status'=> 0,
                            'start' => ($page - 1) * 10,
                            'limit' => 10
                        );
                        $email_total = $this->model_seller_user->getTotalUsers($seller_disabled_data);
                        $results = $this->model_seller_user->getUsers($seller_disabled_data);
                        foreach ($results as $result) {
                            $emails[] = $result['email'];
                        }
