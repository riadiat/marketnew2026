...
class ModelSaleOrder extends Model {
    ...
    public function getOrderHistoriesSeller($order_id, $start = 0, $limit = 10) {
        if ($start < 0) {
            $start = 0;
        }
        ...
        $query = $this->db->query("SELECT oh.date_added, oh.seller_id AS seller_id , os.name AS status, oh.comment, oh.notify FROM " . DB_PREFIX . "order_history_seller oh LEFT JOIN " . DB_PREFIX . "order_status os ON oh.order_status_id = os.order_status_id WHERE oh.order_id = '" . (int)$order_id . "' AND os.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY oh.date_added DESC LIMIT " . (int)$start . "," . (int)$limit);

        return $query->rows;
    }

    public function getTotalOrderHistoriesSeller($order_id) {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "order_history_seller WHERE order_id = '" . (int)$order_id . "'");
