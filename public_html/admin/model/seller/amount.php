<?php
class ModelSellerAmount extends Model {
    ...
    public function getAmount($data = []){
        $sql = "SELECT * , SUM(price) AS price FROM `" . DB_PREFIX . "seller_amount` GROUP BY seller_id";
    ...
    public function getCountAmountOrders($seller_id){
        $sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "seller_amount` WHERE seller_id = '".(int)$seller_id."'";
        $query = $this->db->query($sql);
    ...
    public function getTotalAmount(){
        $sql = "SELECT * , SUM(price) AS total FROM `" . DB_PREFIX . "seller_amount` GROUP BY seller_id";
